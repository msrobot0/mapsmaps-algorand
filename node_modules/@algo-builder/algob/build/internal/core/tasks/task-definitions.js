...
const types = __importStar(require("../params/argument-types"));
const builder_params_1 = require("../params/builder-params");
/**
...
class SimpleTaskDefinition {
    ...
    constructor(name, isInternal = false) {
        ...
        this.action = () => {
            throw new web_1.BuilderError(web_1.ERRORS.TASK_DEFINITIONS.ACTION_NOT_SET, {
                taskName: name
            });
        };
    }
    ...
    addParam(name, description, defaultValue, type, isOptional = defaultValue !== undefined) {
        if (type === undefined) {
            ...
            if (typeof defaultValue !== "string") {
                throw new web_1.BuilderError(web_1.ERRORS.TASK_DEFINITIONS.DEFAULT_VALUE_WRONG_TYPE, {
                    paramName: name,
                    taskName: this.name
                });
            }
    ...
    _validateNotAfterVariadicParam(name) {
        if (this._hasVariadicParam) {
            throw new web_1.BuilderError(web_1.ERRORS.TASK_DEFINITIONS.PARAM_AFTER_VARIADIC, {
                paramName: name,
                taskName: this.name
            });
        }
    ...
    _validateNameNotUsed(name) {
        if (this._hasParamDefined(name)) {
            throw new web_1.BuilderError(web_1.ERRORS.TASK_DEFINITIONS.PARAM_ALREADY_DEFINED, {
                paramName: name,
                taskName: this.name
            });
        }
        if (Object.keys(builder_params_1.ALGOB_PARAM_DEFINITIONS).includes(name)) {
            throw new web_1.BuilderError(web_1.ERRORS.TASK_DEFINITIONS.PARAM_CLASHES_WITH_ALGOB_PARAM, {
                paramName: name,
                taskName: this.name
            });
        }
    ...
    _validateNoMandatoryParamAfterOptionalOnes(name, isOptional) {
        if (!isOptional && this._hasOptionalPositionalParam) {
            throw new web_1.BuilderError(web_1.ERRORS.TASK_DEFINITIONS.MANDATORY_PARAM_AFTER_OPTIONAL, {
                paramName: name,
                taskName: this.name
            });
        }
    ...
    _validateParamNameCasing(name) {
        ...
        if (match === null) {
            throw new web_1.BuilderError(web_1.ERRORS.TASK_DEFINITIONS.INVALID_PARAM_NAME_CASING, {
                paramName: name,
                taskName: this.name
            });
        }
    }
    _validateNoDefaultValueForMandatoryParam(defaultValue, // eslint-disable-line @typescript-eslint/no-explicit-any
    isOptional, name) {
        if (defaultValue !== undefined && !isOptional) {
            throw new web_1.BuilderError(web_1.ERRORS.TASK_DEFINITIONS.DEFAULT_IN_MANDATORY_PARAM, {
                paramName: name,
                taskName: this.name
            });
        }
...
class OverriddenTaskDefinition {
    ...
    addParam(name, description, defaultValue, type, isOptional) {
        if (isOptional === undefined || !isOptional) {
            return this._throwNoParamsOverrideError(web_1.ERRORS.TASK_DEFINITIONS.OVERRIDE_NO_MANDATORY_PARAMS);
        }
    ...
    addPositionalParam(name, description, defaultValue, type, isOptional) {
        return this._throwNoParamsOverrideError(web_1.ERRORS.TASK_DEFINITIONS.OVERRIDE_NO_POSITIONAL_PARAMS);
    }
    ...
    addVariadicPositionalParam(name, description, defaultValue, type, isOptional) {
        return this._throwNoParamsOverrideError(web_1.ERRORS.TASK_DEFINITIONS.OVERRIDE_NO_VARIADIC_PARAMS);
    }
    /**
     * Overriden tasks can't add new parameters.
     */
    ...
    _throwNoParamsOverrideError(errorDescriptor) {
        throw new web_1.BuilderError(errorDescriptor, {
            taskName: this.name
        });
    }
}
