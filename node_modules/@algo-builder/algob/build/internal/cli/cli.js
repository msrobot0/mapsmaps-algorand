...
const log = (0, debug_1.default)("algob:core:cli");
...
function ensureValidNodeVersion(packageJson) {
    ...
    if (!semver_1.default.satisfies(process.version, requirement)) {
        throw new web_1.BuilderError(web_1.ERRORS.GENERAL.INVALID_NODE_VERSION, {
            requirement
        });
    }
}
function printErrRecur(error) {
    if (error.parent) {
        if (error.parent instanceof web_1.BuilderError) {
...
async function gatherArguments() {
    ...
    const envVariableArguments = (0, env_variables_1.getEnvRuntimeArgs)(builder_params_1.ALGOB_PARAM_DEFINITIONS, process.env);
...
async function loadEnvironmentAndArgs(maybeTaskName, runtimeArgs, argumentsParser, unparsedCLAs) {
    ...
    let taskName = maybeTaskName ?? task_names_1.TASK_HELP;
    if (taskDefinitions[taskName] == null) {
        throw new web_1.BuilderError(web_1.ERRORS.ARGUMENTS.UNRECOGNIZED_TASK, {
            task: taskName
        });
    }
    ...
    const isSetup = (0, builtin_tasks_1.isSetupTask)(taskName);
    // Being inside of a project is non-mandatory for help and init
    if (!isSetup && !(0, project_structure_1.isCwdInsideProject)()) {
        throw new web_1.BuilderError(web_1.ERRORS.GENERAL.NOT_INSIDE_PROJECT, { task: origTaskName });
    }
...
async function main() {
    ...
    try {
        ...
        try {
            ...
            if (!(0, exceptions_1.checkAlgorandUnauthorized)(e, env.network)) {
                throw e;
            }
        }
        ...
        if (web_1.BuilderError.isBuilderError(error)) {
            console.error(chalk_1.default.red(`Error ${error.message}`)); // eslint-disable-line @typescript-eslint/restrict-template-expressions
        }
        else if (errors_1.BuilderPluginError.isBuilderPluginError(error)) {
            console.error(chalk_1.default.red(`Error in plugin ${error.pluginName ?? ""}: ${error.message}`) // eslint-disable-line @typescript-eslint/restrict-template-expressions
            );
        }
