...
class ArgumentsParser {
    ...
    static cLAToParamName(cLA) {
        if (cLA.toLowerCase() !== cLA) {
            throw new web_1.BuilderError(web_1.ERRORS.ARGUMENTS.PARAM_NAME_INVALID_CASING, {
                param: cLA
            });
        }
        ...
        for (let i = 0; i < rawCLAs.length; i++) {
            ...
            if (taskName === undefined) {
                ...
                if (!this._isCLAParamName(arg, paramDefs)) {
                    throw new web_1.BuilderError(web_1.ERRORS.ARGUMENTS.UNRECOGNIZED_COMMAND_LINE_ARG, { argument: arg });
                }
            }
            else {
                if (!this._isCLAParamName(arg, paramDefs)) {
    ...
    _parseTaskParamArguments(taskDefinition, rawCLAs) {
        ...
        for (let i = 0; i < rawCLAs.length; i++) {
            ...
            if (!this._isCLAParamName(arg, taskDefinition.paramDefinitions)) {
                throw new web_1.BuilderError(web_1.ERRORS.ARGUMENTS.UNRECOGNIZED_PARAM_NAME, {
                    param: arg
                });
            }
    ...
    _addTaskDefaultArguments(taskDefinition, taskArguments) {
        for (const paramName of Object.keys(taskDefinition.paramDefinitions)) {
            ...
            if (!definition.isOptional) {
                throw new web_1.BuilderError(web_1.ERRORS.ARGUMENTS.MISSING_TASK_ARGUMENT, {
    ...
    _parseArgumentAt(rawCLAs, index, paramDefinitions, parsedArguments) {
        ...
        if (parsedArguments[paramName] !== undefined) {
            throw new web_1.BuilderError(web_1.ERRORS.ARGUMENTS.REPEATED_PARAM, {
                param: claArg
            });
        }
    ...
    _parsePositionalParamArgs(rawPositionalParamArgs, positionalParamDefinitions // eslint-disable-line @typescript-eslint/no-explicit-any
        ...
        for (let i = 0; i < positionalParamDefinitions.length; i++) {
            ...
            if (rawArg === undefined) {
                if (!definition.isOptional) {
                    throw new web_1.BuilderError(web_1.ERRORS.ARGUMENTS.MISSING_POSITIONAL_ARG, {
        ...
        const hasVariadicParam = positionalParamDefinitions[positionalParamDefinitions.length - 1]?.isVariadic;
        ;
        if (!hasVariadicParam &&
            rawPositionalParamArgs.length > positionalParamDefinitions.length) {
            throw new web_1.BuilderError(web_1.ERRORS.ARGUMENTS.UNRECOGNIZED_POSITIONAL_ARG, {
