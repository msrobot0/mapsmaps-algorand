...
function lazyObject(objectCreator) {
    return createLazyProxy(objectCreator, (getRealTarget) => ({
        ...
        if (object instanceof Function) {
            throw new web_1.BuilderError(web_1.ERRORS.GENERAL.UNSUPPORTED_OPERATION, {
...
function lazyFunction(functionCreator) {
    return createLazyProxy(functionCreator, (getRealTarget) => {
        ...
        dummyTarget[util.inspect.custom] = function () {
            const realTarget = getRealTarget();
            return util.inspect(realTarget);
        };
        return dummyTarget;
...
function createLazyProxy(// eslint-disable-line
targetCreator, dummyTargetCreator, validator // eslint-disable-line @typescript-eslint/no-explicit-any
) {
    let realTarget;
    // tslint:disable-next-line
    const dummyTarget = dummyTargetCreator(getRealTarget); // eslint-disable-line @typescript-eslint/no-explicit-any
    ...
    const handler = {
        ...
        get(target, property, receiver) {
            ...
            if (stack?.includes("givenProvider.js") &&
    ...
    if (dummyTarget instanceof Function) {
        ...
        handler.apply = (target, thisArg, argArray) => {
            // tslint:disable-next-line ban-types
            return Reflect.apply(getRealTarget(), thisArg, argArray); // eslint-disable-line
        };
        handler.construct = (target, argArray, newTarget) => {
            // tslint:disable-next-line ban-types
            return Reflect.construct(getRealTarget(), argArray); // eslint-disable-line
        };
    }
