export declare const TASK_HELP = "help";
export declare const TASK_CONSOLE = "console";
export declare const TASK_CLEAN = "clean";
export declare const TASK_GEN_ACCOUNTS = "gen-accounts";
export declare const TASK_INIT = "init";
export declare const TASK_NODE_INFO = "node-info";
export declare const TASK_RUN = "run";
export declare const TASK_DEPLOY = "deploy";
export declare const TASK_COMPILE = "compile";
export declare const TASK_UNBOX_TEMPLATE = "unbox-template";
export declare const TASK_TEST = "test";
export declare const TASK_SIGN_MULTISIG = "sign-multisig";
export declare const TASK_SIGN_LSIG = "sign-lsig";
