...
const opcode_list_1 = require("./opcode-list");
/**
...
class Interpreter {
    constructor() {
        ...
        this.intcblock = [];
        // scratch spece used
        this.scratch = new Array(256).fill(constants_1.DEFAULT_STACK_ELEM);
        this.instructions = [];
     ...
     */
    _getAccountFromAddr(accountPk, line) {
        ...
        if (this.tealVersion <= 3) {
            ...
            throw new runtime_errors_1.RuntimeError(errors_list_1.RUNTIME_ERRORS.TEAL.PRAGMA_VERSION_ERROR, {
                expected: constants_1.MaxTEALVersion,
        ...
        const addr = (0, algosdk_1.encodeAddress)(pkBuffer);
        if (!(0, algosdk_1.isValidAddress)(addr)) { // invalid address
            throw new runtime_errors_1.RuntimeError(errors_list_1.RUNTIME_ERRORS.TEAL.ADDR_NOT_VALID, {
                address: addr,
                line: line
            });
        }
        if (txAccounts?.find(buff => (0, compare_1.compareArray)(Uint8Array.from(buff), accountPk)) !== undefined ||
            (0, compare_1.compareArray)(accountPk, Uint8Array.from(this.runtime.ctx.tx.snd)) ||
            // since tealv5, currentApplicationAddress is also allowed (directly)
            (0, compare_1.compareArray)(accountPk, (0, algosdk_1.decodeAddress)((0, algosdk_1.getApplicationAddress)(appID)).publicKey)) {
            const address = (0, algosdk_1.encodeAddress)(pkBuffer);
            const account = this.runtime.ctx.state.accounts.get(address);
            return this.runtime.assertAccountDefined(address, account, line);
        }
        else {
            throw new runtime_errors_1.RuntimeError(errors_list_1.RUNTIME_ERRORS.TEAL.ADDR_NOT_FOUND_IN_TXN_ACCOUNT, {
                address: (0, algosdk_1.encodeAddress)(pkBuffer),
                line: line
            });
        }
    }
     ...
     */
    getAccount(accountRef, line) {
        let account;
        let address;
        if (typeof accountRef === 'bigint') {
            if (accountRef === 0n) {
                address = (0, algosdk_1.encodeAddress)(this.runtime.ctx.tx.snd);
                account = this.runtime.ctx.state.accounts.get(address);
            }
            else {
                const accIndex = accountRef - 1n;
                (0, compare_1.checkIndexBound)(Number(accIndex), this.runtime.ctx.tx.apat ?? [], line);
                let pkBuffer;
     ...
     * https://developer.algorand.org/articles/introducing-algorand-virtual-machine-avm-09-release/
     */
    getAppIDByReference(appRef, foreign, line, op) {
        ...
        throw new runtime_errors_1.RuntimeError(errors_list_1.RUNTIME_ERRORS.TEAL.INVALID_APP_REFERENCE, {
            appRef: appRef,
            line: line
        });
    }
     ...
     * https://developer.algorand.org/articles/introducing-algorand-virtual-machine-avm-09-release/
     */
    getAssetIDByReference(assetRef, foreign, line, op) {
        ...
        throw new runtime_errors_1.RuntimeError(errors_list_1.RUNTIME_ERRORS.TEAL.INVALID_ASA_REFERENCE, {
            assetRef: assetRef,
            line: line
        });
    }
    ...
    setGlobalState(appID, key, value, line) {
        if (!this.runtime.ctx.state.globalApps.has(appID)) {
            throw new runtime_errors_1.RuntimeError(errors_list_1.RUNTIME_ERRORS.GENERAL.APP_NOT_FOUND, { appID: appID, line: line });
        }
    ...
    jumpForward(label, line) {
        while (++this.instructionIndex < this.instructions.length) {
            ...
            if (instruction instanceof opcode_list_1.Label && instruction.label === label) {
                ...
                for (; this.instructionIndex < this.instructions.length - 1; ++this.instructionIndex) {
                    ...
                    if (!(nextInstruction instanceof opcode_list_1.Label)) {
                        break;
                    }
                }
                return;
        ...
        throw new runtime_errors_1.RuntimeError(errors_list_1.RUNTIME_ERRORS.TEAL.LABEL_NOT_FOUND, {
            label: label,
            line: line
        });
    }
     ...
     */
    printStack(instruction, debugStack) {
        if (!debugStack) {
            return;
        }
        console.info("stack(depth = %s) for opcode %s at line %s:", debugStack, instruction.constructor.name, instruction.line);
    ...
    mapLabelWithIndexes() {
        this.instructions.forEach((instruction, idx) => {
            if (instruction instanceof opcode_list_1.Label) {
    ...
    assertValidTxArray() {
        ...
        if (appArgsLen > constants_1.ALGORAND_MAX_APP_ARGS_LEN) {
            throw new runtime_errors_1.RuntimeError(errors_list_1.RUNTIME_ERRORS.GENERAL.INVALID_APP_ARGS_LEN, {
                len: appArgsLen,
                max: constants_1.ALGORAND_MAX_APP_ARGS_LEN
            });
        }
        if (txAccountsLen > constants_1.ALGORAND_MAX_TX_ACCOUNTS_LEN) {
            throw new runtime_errors_1.RuntimeError(errors_list_1.RUNTIME_ERRORS.GENERAL.INVALID_TX_ACCOUNTS_LEN, {
                len: txAccountsLen,
                max: constants_1.ALGORAND_MAX_TX_ACCOUNTS_LEN
            });
        }
        const totalLen = txAccountsLen + foreignAppsLen + foreignAssetsLen;
        if (totalLen > constants_1.ALGORAND_MAX_TX_ARRAY_LEN) {
            throw new runtime_errors_1.RuntimeError(errors_list_1.RUNTIME_ERRORS.GENERAL.MAX_REFERENCES_EXCEEDED, {
                len: totalLen,
                max: constants_1.ALGORAND_MAX_TX_ARRAY_LEN
            });
        }
    }
    /**
     ...
     */
    execute(program, mode, runtime, debugStack) {
     ...
     *       the result is zero.
     */
    executeWithResult(program, mode, runtime, debugStack) {
        this.runtime = runtime;
        this.instructions = (0, parser_1.parser)(program, mode, this);
        this.mapLabelWithIndexes();
        if (mode === types_1.ExecutionMode.APPLICATION) {
        ...
        while (this.instructionIndex < this.instructions.length) {
            ...
            const txReceipt = this.runtime.ctx.state.txReceipts.get(this.runtime.ctx.tx.txID);
            ...
            if (this.tealVersion >= 4) {
                if (mode === types_1.ExecutionMode.SIGNATURE) {
                    (0, parser_1.assertMaxCost)(dynamicCost, mode);
                    txReceipt.gas = dynamicCost;
                }
                else {
                    this.runtime.ctx.pooledApplCost += this.lineToCost[instruction.line];
                    const maxPooledApplCost = constants_1.MaxAppProgramCost * this.runtime.ctx.gtxs.length;
                    (0, parser_1.assertMaxCost)(this.runtime.ctx.pooledApplCost, types_1.ExecutionMode.APPLICATION, maxPooledApplCost);
