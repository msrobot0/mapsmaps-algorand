...
exports.BaseModelI = exports.AccountStore = exports.RuntimeAccount = void 0;
...
const stateful_1 = require("./lib/stateful");
...
exports.RuntimeAccount = RuntimeAccount;
class AccountStore {
    constructor(balance, account) {
        if (typeof account === 'string') {
            // create new account with name
            this.account = new RuntimeAccount((0, algosdk_1.generateAccount)(), account);
        }
        else if (account) {
            ...
            this.account = new RuntimeAccount((0, algosdk_1.generateAccount)());
        }
        this.address = this.account.addr;
        this.assets = new Map();
        this.amount = BigInt(balance);
        this.minBalance = constants_1.ALGORAND_ACCOUNT_MIN_BALANCE;
        this.appsLocalState = new Map();
        this.appsTotalSchema = {};
        this.createdApps = new Map();
        this.createdAssets = new Map();
    }
     ...
     */
    setLocalState(appID, key, value, line) {
        ...
        if (localState && localApp) {
            ...
            (0, stateful_1.assertValidSchema)(localState[StateMap], localState.schema); // verify if updated schema is valid by config
            return localState;
        }
    ...
    getGlobalState(appID, key) {
        ...
        const globalKey = (0, parsing_1.keyToBytes)(key);
     ...
     */
    setGlobalState(appID, key, value, line) {
        const app = this.getApp(appID);
        if (app === undefined)
            throw new runtime_errors_1.RuntimeError(errors_list_1.RUNTIME_ERRORS.GENERAL.APP_NOT_FOUND, { appID: appID, line: line ?? 'unknown' });
        const appGlobalState = app[globalState];
        const globalKey = (0, parsing_1.keyToBytes)(key);
        ...
        (0, stateful_1.assertValidSchema)(app[globalState], app[globalStateSchema]); // verify if updated schema is valid by config
    }
    /**
     ...
     */
    addAsset(assetId, name, asaDef) {
        if (this.createdAssets.size === constants_1.MAX_ALGORAND_ACCOUNT_ASSETS) {
            throw new runtime_errors_1.RuntimeError(errors_list_1.RUNTIME_ERRORS.ASA.MAX_LIMIT_ASSETS, { name: name, address: this.address, max: constants_1.MAX_ALGORAND_ACCOUNT_ASSETS });
        }
        this.minBalance += constants_1.ASSET_CREATION_FEE;
    ...
    modifyAsset(assetId, fields) {
        ...
        (0, asa_1.checkAndSetASAFields)(fields, asset);
    }
    /**
     ...
     */
    setFreezeState(assetId, state) {
        const holding = this.assets.get(assetId);
        if (holding === undefined) {
            throw new runtime_errors_1.RuntimeError(errors_list_1.RUNTIME_ERRORS.TRANSACTION.ASA_NOT_OPTIN, { address: this.address, assetId: assetId });
        }
    ...
    destroyAsset(assetId) {
        ...
        if (holding.amount !== asset.total) {
            throw new runtime_errors_1.RuntimeError(errors_list_1.RUNTIME_ERRORS.ASA.ASSET_TOTAL_ERROR);
        }
        this.minBalance -= constants_1.ASSET_CREATION_FEE;
     ...
     */
    addApp(appID, params, approvalProgram, clearProgram) {
        if (this.createdApps.size === constants_1.MAX_ALGORAND_ACCOUNT_CREATED_APPS) {
            throw new runtime_errors_1.RuntimeError(errors_list_1.RUNTIME_ERRORS.GENERAL.MAX_LIMIT_APPS, {
                address: this.address,
                max: constants_1.MAX_ALGORAND_ACCOUNT_CREATED_APPS
            });
        }
        ;
        ...
        this.minBalance += (constants_1.APPLICATION_BASE_FEE +
            (constants_1.SSC_VALUE_UINT * params.globalInts) + (constants_1.SSC_VALUE_BYTES * params.globalBytes));
        const app = new App(appID, params, approvalProgram, clearProgram);
        this.createdApps.set(app.id, app.attributes);
        return app;
    }
    // opt in to application
    optInToApp(appID, appParams) {
        ...
        if (localState) {
            ...
            if (this.appsLocalState.size === constants_1.MAX_ALGORAND_ACCOUNT_OPTEDIN_APPS) {
                throw new Error(`Maximum Opt In applications per account is ${constants_1.MAX_ALGORAND_ACCOUNT_OPTEDIN_APPS}`);
            }
            // https://developer.algorand.org/docs/features/asc1/stateful/#minimum-balance-requirement-for-a-smart-contract
            this.minBalance += (constants_1.APPLICATION_BASE_FEE +
                (constants_1.SSC_VALUE_UINT * Number(appParams[localStateSchema].numUint)) +
                (constants_1.SSC_VALUE_BYTES * Number(appParams[localStateSchema].numByteSlice)));
    ...
    // opt-in to asset
    optInToASA(assetIndex, assetHolding) {
        ...
        if (accAssetHolding) {
            ...
            if ((this.createdAssets.size + this.assets.size) === constants_1.MAX_ALGORAND_ACCOUNT_ASSETS) {
                throw new runtime_errors_1.RuntimeError(errors_list_1.RUNTIME_ERRORS.ASA.MAX_LIMIT_ASSETS, { address: assetHolding.creator, max: constants_1.MAX_ALGORAND_ACCOUNT_ASSETS });
            }
            this.minBalance += constants_1.ASSET_CREATION_FEE;
    ...
    deleteApp(appID) {
        ...
        this.minBalance -= (constants_1.APPLICATION_BASE_FEE +
            (constants_1.SSC_VALUE_UINT * Number(app[globalStateSchema].numUint)) +
            (constants_1.SSC_VALUE_BYTES * Number(app[globalStateSchema].numByteSlice)));
    ...
    closeApp(appID) {
        ...
        this.minBalance -= (constants_1.APPLICATION_BASE_FEE +
            (constants_1.SSC_VALUE_UINT * Number(localApp.schema.numUint)) +
            (constants_1.SSC_VALUE_BYTES * Number(localApp.schema.numByteSlice)));
...
class App {
    // NOTE - approval and clear program must be the TEAL code as string
    constructor(appID, params, approvalProgram, clearProgram) {
        this.id = appID;
        const base = new BaseModelI();
        this.attributes = {
...
// represents asset
class Asset {
    constructor(assetId, def, creator, assetName) {
        this.id = assetId;
        const base = new BaseModelI();
        this.definitions = {
            ...base,
